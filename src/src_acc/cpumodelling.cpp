...
void FDOperator_4(int it, int isource)
{
    ...
    if(NJ == 4)
    { 
        #pragma  acc parallel loop independent present(der1, der2)
        ...
        if(it <= nts)
        {
            #pragma acc parallel loop independent firstprivate (isource, fd_dt, it, dx, dz)
            for(ompi=0; ompi<1; ompi++)
            {
...
void modelling_module(int my_nsrc, geo2d_t *mygeo2d_sp)
{
    ...
    #pragma acc enter data copyin(damp1a[0:nnz], damp2a[0:nnx], damp2b[0:nnx], damp1b[0:nnz], damp2a1[0:nnx], damp2b1[0:nnx], damp1a1[0:nnz], damp1b1[0:nnz], spg[0:npml+1], spg1[0:npml+1], kappaw1[0:N], kappaw2[0:N], buw1[0:N], buw2[0:N], kappa[0:N], bu1[0:N], bu2[0:N], vx[0:N], der2[0:N], vz[0:N], der1[0:N], px[0:N], pz[0:N], p[0:N])    
     
    #pragma  acc parallel loop independent present(damp2a, damp2b, damp2a1, damp2b1)
    ...
    #pragma  acc parallel loop independent present(damp1a, damp1b, damp1a1, damp1b1)
    ...
    #pragma acc parallel loop present (kappaw1, kappaw2, buw1, buw2)    
    for(ompi = 0; ompi < N; ompi++)
    {
    ...
    #pragma acc parallel loop present (kappaw1, kappaw2, buw1, buw2, kappa, damp2a1, damp1a1, bu2, damp2b1, bu1, damp1b1)	
    for(ix = 0; ix < nnx; ix++)
    {
    ...
    #pragma acc exit data delete(damp2a1, damp2b1, damp1a1, damp1b1, spg, spg1, bu2, bu1)
    ...
    for(is = istart; is < ns; is++)
    {
        ...
        #pragma acc parallel loop independent present(p, px, pz, vx, vz)
        for(ompi = 0; ompi < N; ompi++)
        {
            p[ompi] = 0.0;          px[ompi] = 0.0;         pz[ompi] = 0.0;
            vx[ompi] = 0.0;         vz[ompi] = 0.0; 
        }
	    #pragma acc enter data copyin(h_temp[0:ng*real_nt], h_Gxz[0:ng]) 
        for(kt = 0; kt < fd_nt; kt++)
        {
            ...
            if( (kt+1)%dt_factor == 0 )
            {
                ...
                #pragma acc parallel loop independent present(p, h_Gxz, h_temp)
                for(ig = 0; ig < ng; ig++)
                    h_temp[indx*ng+ig] = (float) p[h_Gxz[ig]];
            }
        ...
        #pragma acc exit data copyout(h_temp[0:ng*real_nt])
        ...
        #pragma acc exit data delete(h_Gxz)
    ...
    #pragma acc exit data delete(damp1a, damp2a, damp2b, damp1b, kappaw1, kappaw2, buw1, buw2, kappa, vx, der2, vz, der1, px, pz, p)	
