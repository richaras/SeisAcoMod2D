...
void read_source2d(char *shot_file)
{
	...
	{
		//sx=mtr    sz=mtr    Number_of_reciever    Near_offset    receiver spacing    Geometry_type
				
        //fscanf(fp_src, "%f %f %d %f %f %d", &sx, &sz, &nrec, &nearoff, &ngeoph, &geotype);
		...
		//Set receiver for each shot depend on information readed
